...
int tool_menu_items_comparator(const void* a, const void* b)
{
    ...
    if (first->set == false && second->set == false)
        return 0;
    if (first->set == false)
        return 1;
    if (second->set == false)
    ...
    if (first->order[i] < second->order[i])
        return -1;
    else if (first->order[i] > second->order[i])
        return 1;
    else
        return 0;
}
...
void tool_finalize()
{
    ...
    unsigned short int order = 1;
        ...
        for
            ALL(tools_state.menu_items, menu_items_iterator)
            {
...
void tool_create_label(const char* const title)
{
    ...
    tools_state.tools[n].tab_last = gui_new_label(tools_state.tools[n].tab, tools_state.tools[n].tab_last, title);
...
void tool_fetch_parameters(size_t tool_id)
{
    ...
    // go through all parameters of this tool and save it
        for
            ALL(tools_state.tools[tool_id].parameters, i)
            {
