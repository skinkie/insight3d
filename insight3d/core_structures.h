...
#define INDEX_DECLARATION(indexing_variable) \
    size_t indexing_variable;                \
    bool indexing_variable##_set
#define INDEX_CLEAR(indexing_variable) \
    (indexing_variable) = 0;           \
    (indexing_variable##_set) = false
#define INDEX_SET(indexing_variable, value) \
    (indexing_variable) = (value);          \
    (indexing_variable##_set) = true
...
#define LAMBDA_FROM(variable_name, from, iterator, function)                    \
    for ((iterator) = (from); (iterator) < (variable_name).count; ++(iterator)) \
        if ((variable_name).data[(iterator)].set) {                             \
            function                                                            \
        }
...
#define DYNAMIC_STRUCTURE(structure_name, structure_type)                                                                                                  \
    ...
    void dyn_free(structure_name* dynamic_structure)                                                                                                       \
    {                                                                                                                                                      \
        if (dynamic_structure->data)                                                                                                                       \
            free(dynamic_structure->data);                                                                                                                 \
        dyn_initialize(dynamic_structure);                                                                                                                 \
    }                                                                                                                                                      \
                                                                                                                                                           \
    structure_name* dyn(structure_name* dynamic_structure, size_t index)                                                                                   \
    {                                                                                                                                                      \
        if (dynamic_structure->count <= index)                                                                                                             \
            dynamic_structure->count = index + 1;                                                                                                          \
                                                                                                                                                           \
    ...
    size_t dyn_next(const structure_name& dynamic_structure, const size_t from)                                                                            \
    {                                                                                                                                                      \
        for (size_t i = from; i < dynamic_structure.count; ++i)                                                                                            \
            if (dynamic_structure.data[i].set) {                                                                                                           \
                return i;                                                                                                                                  \
            }                                                                                                                                              \
                                                                                                                                                           \
        return SIZE_MAX;                                                                                                                                   \
