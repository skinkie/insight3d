...
bool ui_add_point_to_selection(const size_t shot_id, const size_t point_id)
{
    ...
    if (shots.data[shot_id].points.data[point_id].selected)
        return true;
...
void ui_select_points_with_reconstructed_vertices()
{
    if (INDEX_IS_SET(ui_state.current_calibration)) {
        // go through all vertices reconstructed in this calibration
                for
                    ALL(calibrations.data[ui_state.current_calibration].Xs, i)
                    {
                        ...
                        for
                            ALL(vertices_incidence.data[vertex->vertex_id].shot_point_ids, j)
                            {
        ...
        // remove vertices inside selection box from current selection (i.e. 'remove operation')
                for
                    ALL(ui_state.selection_list, i)
                    {
                        ...
                        if (selected_item->item_type != GEOMETRY_VERTEX)
                            continue;
                        ...
                        // 3d selection box applies only to vertices with reconstructed 3d position
                        if (!vertex->reconstructed || ui_vertex_invisible(selected_item->item_id))
                            continue;
...
void ui_delete_selected_points(bool dont_restrict_to_current_shot /*= false*/)
{
        for
            ...
            {
                if (
                    (ui_state.selection_list.data[i].item_type == GEOMETRY_POINT) && (dont_restrict_to_current_shot || ui_state.selection_list.data[i].shot_id == ui_state.current_shot)) {
