...
void ui_event_redraw()
{
    ...
    case UI_MODE_SHOT:
        ...
        if (shots.data[ui_state.current_shot].info_status >= GEOMETRY_INFO_DEDUCED && gui_get_width(ui_state.gl) > 0 && gui_get_height(ui_state.gl) > 0) {
            ...
            if (!dualview_displayed) {
                ...
                if (ui_state.mouse_over)
                    ui_context_display(ui_state.tool_x, ui_state.tool_y);
            }
        } else {
            ui_release_dualview();
        }
    ...
    if (INDEX_IS_SET(ui_state.current_shot)) {
        if (ui_state.mouse_down && (ui_state.mode == UI_MODE_SHOT && ui_state.mouse_button == SDL_BUTTON_RIGHT /*|| 
...
void ui_event_mouse_button_down(Uint8 button, Uint16 x, Uint16 y)
{
    ...
    if (tools_state.tools[current_tool].mode_affinity == ui_state.mode || tools_state.tools[current_tool].mode_affinity == UI_MODE_UNSPECIFIED) {
        ...
        if (
            tools_state.tools[current_tool].mouse_down && (!tools_state.tools[current_tool].process_events || tools_state.tools[current_tool].process_events())) {
...
void ui_event_mouse_move(Uint16 x, Uint16 y)
{
    ...
    if (tools_state.tools[current_tool].mode_affinity == ui_state.mode || tools_state.tools[current_tool].mode_affinity == UI_MODE_UNSPECIFIED) {
        ...
        if (!ui_state.mouse_down) {
            ...
            if (
                tools_state.tools[current_tool].move && (!tools_state.tools[current_tool].process_events || tools_state.tools[current_tool].process_events())) {
            ...
            if (
                tools_state.tools[current_tool].dragging && (!tools_state.tools[current_tool].process_events || tools_state.tools[current_tool].process_events())) {
...
void ui_event_mouse_button_up(Uint8 button, Uint16 x, Uint16 y)
{
    ...
    if (
        SDL_GetTicks() - ui_state.mouse_down_ticks < UI_CLICK_DELAY && sqr_value(ui_state.mouse_down_x - ui_state.mouse_x) + sqr_value(ui_state.mouse_down_y - ui_state.mouse_y) < UI_CLICK_DISTANCE_SQ) {
        // * click *
        ...
        if (tools_state.tools[current_tool].mode_affinity == ui_state.mode || tools_state.tools[current_tool].mode_affinity == UI_MODE_UNSPECIFIED) {
            ...
            if (
                tools_state.tools[current_tool].click && (!tools_state.tools[current_tool].process_events || tools_state.tools[current_tool].process_events())) {
        ...
        double
            x1
            = min_value(ui_state.tool_down_x, ui_state.tool_x),
        ...
        if (tools_state.tools[current_tool].mode_affinity == ui_state.mode || tools_state.tools[current_tool].mode_affinity == UI_MODE_UNSPECIFIED) {
            ...
            if (!ui_state.mouse_no_dragging) {
                ...
                if (
                    tools_state.tools[current_tool].dragging_done && (!tools_state.tools[current_tool].process_events || tools_state.tools[current_tool].process_events())) {
...
void ui_event_update(const Uint32 delta_time)
{
    ...
    switch (ui_state.mode) {
    case UI_MODE_SHOT:
        ui_event_update_shot(delta_time);
        break;
    case UI_MODE_INSPECTION:
...
void ui_event_update_shot(const Uint32 delta_time)
{
    ...
    if (tools_state.tools[current_tool].mode_affinity == UI_MODE_SHOT || tools_state.tools[current_tool].mode_affinity == UI_MODE_UNSPECIFIED) {
        ...
        if (
            tools_state.tools[current_tool].key_pressed && (!tools_state.tools[current_tool].process_events || tools_state.tools[current_tool].process_events())) {
