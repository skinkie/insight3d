...
)
{
    ...
    for (int j = 0; j < n; j++) {
        ...
        // normalize the row
        if (normalize_A) {
            for (int k = norm = 0; k < 3; k++) {
                norm += OPENCV_ELEM(A, 2 * j, k) * OPENCV_ELEM(A, 2 * j, k);
            }
            norm += OPENCV_ELEM(b, 2 * j, 0) * OPENCV_ELEM(b, 2 * j, 0);
            norm = 1 / sqrt(norm);
            for (int k = 0; k < 3; k++) {
                OPENCV_ELEM(A, 2 * j, k) *= norm;
            }
        ...
        if (normalize_A) {
            for (int k = norm = 0; k < 3; k++) {
                norm += OPENCV_ELEM(A, 2 * j + 1, k) * OPENCV_ELEM(A, 2 * j + 1, k);
            }
...
)
{
    ...
    for (int i = 0; i < trials; i++) {
        ...
        CvMat* X = affine ? mvg_triangulation_SVD_affine(projection_matrices, projected_points, normalize_A, 2, samples, 2)
        ...
        for (int j = 0; j < n; j++) {
            ...
            const double
                dx
                = reprojection[0] - OPENCV_ELEM(projected_points, 0, j),
    ...
    if (outliers == 0)
        min = min_inliers_to_triangulate_weaker;
