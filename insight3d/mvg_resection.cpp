...
)
{
    ...
    if (!vertices || !projected || !P)
        return false;
    if (n < 6)
        return false;
    if (vertices->cols != projected->cols)
        return false;
...
)
{
    ...
    for (int i = 0; i < trials; i++) {
        ...
        if (!mvg_resection_SVD(vertices, projected, P, NULL, NULL, NULL, normalize_A, samples, 6))
            continue;
        ...
        for (int j = 0; j < n; j++) {
            ...
            const double
                dx
                = reprojection[0] - OPENCV_ELEM(projected, 0, j),
...
bool mvg_restrict_calibration_matrix(CvMat* const K, const bool zero_skew, const bool square_pixels)
{
    ...
    const double
        ratio
        = OPENCV_ELEM(K, 0, 0) / OPENCV_ELEM(K, 1, 1),
