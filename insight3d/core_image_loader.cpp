        ...
        if (shot->full || shot->low) {
            ...
            if (achieved_quality > request->current_quality) {
                ...
                if (img) {
                    ...
                    int
                        min_xi
                        = (int)(min_x / shot->width * img->width),
...
{
    ASSERT_IS_SET(image_loader_requests, handle.id);
    return image_loader_requests.data[handle.id].done
...
)
{
    ...
    *full_texture = 0;
    ...
    if (request->content == IMAGE_LOADER_ALL) {
        ...
        switch (request->quality) {
        case IMAGE_LOADER_LOW_RESOLUTION:
            if (shot->low_texture != 0) {
                *low_texture = shot->low_texture;
                result = true;
            }
            break;
        case IMAGE_LOADER_CONTINUOUS_LOADING:
        ...
        if (texture_min_x)
            *texture_min_x = request->gl_texture_min_x;
        if (texture_min_y)
            *texture_min_y = request->gl_texture_min_y;
        if (texture_max_x)
            *texture_max_x = request->gl_texture_max_x;
        if (texture_max_y)
            *texture_max_y = request->gl_texture_max_y;
...
void image_loader_cancel_all_requests()
{
        ...
        for
            ALL(image_loader_shots, i)
            {
                Image_Loader_Shot* const shot = image_loader_shots.data + i;
                if (shot->full)
                    cvReleaseImage(&shot->full);
                if (shot->low)
                    cvReleaseImage(&shot->low);
            }
