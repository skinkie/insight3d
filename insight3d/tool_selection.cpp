...
void tool_selection_key()
{
    if (ui_state.key_state[SDLK_SPACE] && INDEX_IS_SET(ui_state.current_shot)) {
        ...
        memset(visible_vertices_point_id, 0, sizeof(size_t) * vertices.count);
                for
                    ALL(shots.data[ui_state.current_shot].points, i)
                    {
                    ...
                    ALL(polygons, i)
                    {
                        ...
                        for
                            ALL(polygon->vertices, j)
                            {
                                ...
                                for
                                    ALL(vertices_incidence.data[vertex_id].shot_point_ids, k)
                                    {
                                ...
                                if (count[k] == max) {
                                    ...
                                    if (polygon_mat) {
                                        ...
                                        size_t points_count = 0;
                                                        for
                                                            ALL(shots.data[k].points, j)
                                                            {
                                                                Point* point = shots.data[k].points.data + j;
                                                                if (!visible_vertices[point->vertex])
                                                                    continue;
                ...
                if (best_found) {
                    ...
                    size_t
                        *points1_id
                        = ALLOC(size_t, shots.data[ui_state.current_shot].points.count + 128),
                    ...
                    size_t count_manually_entered = 0;
                        for
                            ALL(polygons.data[best_polygon_id].vertices, i)
                            {
                        ...
                        if (polygon_mat) {
                                for
                                    ALL(shots.data[best_shot_id].points, j)
                                    {
                                        Point* point = shots.data[best_shot_id].points.data + j;
                                        if (!visible_vertices[point->vertex] || polygon_vertices[point->vertex])
                                            continue;
                        ...
                        if (count_manually_entered + count_tracked >= 4) {
                            ...
                            while (trials++ < 60) {
                                ...
                                for (size_t n = 0; n < count_tracked + count_manually_entered; n++) {
                                    // project the point
                                    const double w = OPENCV_ELEM(H, 2, 0) * shots.data[best_shot_id].points.data[points1_id[n]].x + OPENCV_ELEM(H, 2, 1) * shots.data[best_shot_id].points.data[points1_id[n]].y + OPENCV_ELEM(H, 2, 2);

                                    if (!nearly_zero(w)) {
                                        const double
                                            x
                                            = (OPENCV_ELEM(H, 0, 0) * shots.data[best_shot_id].points.data[points1_id[n]].x + OPENCV_ELEM(H, 0, 1) * shots.data[best_shot_id].points.data[points1_id[n]].y + OPENCV_ELEM(H, 0, 2)) / w,
                                            y = (OPENCV_ELEM(H, 1, 0) * shots.data[best_shot_id].points.data[points1_id[n]].x + OPENCV_ELEM(H, 1, 1) * shots.data[best_shot_id].points.data[points1_id[n]].y + OPENCV_ELEM(H, 1, 2)) / w;

                                        const double
                                            dx
                                            = shots.data[ui_state.current_shot].width * (x - shots.data[ui_state.current_shot].points.data[points2_id[n]].x),
                                ...
                                // now save estimated points
                                        for
                                            ALL(polygons.data[best_polygon_id].vertices, i)
                                            {
                                                ...
                                                const double w = OPENCV_ELEM(H, 2, 0) * shot->points.data[point_id].x + OPENCV_ELEM(H, 2, 1) * shot->points.data[point_id].y + OPENCV_ELEM(H, 2, 2);

                                                if (!nearly_zero(w)) {
                                                    const double
                                                        x
                                                        = (OPENCV_ELEM(H, 0, 0) * shot->points.data[point_id].x + OPENCV_ELEM(H, 0, 1) * shot->points.data[point_id].y + OPENCV_ELEM(H, 0, 2)) / w,
                                                        y = (OPENCV_ELEM(H, 1, 0) * shot->points.data[point_id].x + OPENCV_ELEM(H, 1, 1) * shot->points.data[point_id].y + OPENCV_ELEM(H, 1, 2)) / w;
...
bool tool_selection_mouse_down(double x, double y, int button)
{
    ...
    case SDL_MOUSEWHEEL:
        if (y >= 0) {
            ...
            const double
                point_under_cursor_x
                = meta->view_center_x + (x - meta->view_center_x) / TOOL_SELECTION_ZOOM_RATE,
        ...
        if (!INDEX_IS_SET(ui_state.focused_point) || point_id != ui_state.focused_point) {
            ...
            if (query_count_points_by_vertex(vertex_id) > 1) {
                ...
                // fill it with thumbnails
                                for
                                    ALL(vertices_incidence.data[vertex_id].shot_point_ids, i)
                                    {
...
void debug_save_initial_solution()
{
    ...
    size_t
        *index_Ps
        = dyn_build_reindex(calibration->Ps),
    ...
    std::ofstream cameras_out("./cameras.txt");
        ...
        for
            ALL(calibration->Xs, i)
            {
                ...
                size_t points_count = 0;
                for
                    ALL(vertices_incidence.data[X->vertex_id].shot_point_ids, j)
                    {
