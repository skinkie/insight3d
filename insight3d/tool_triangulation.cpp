                    ...
                    // find these two points among the ones in the pictures
                                for
                                    ALL(shot->points, j)
                                    {
                                        ...
                                        if (nearly_zero(
                                                sqr_value(org->pt.x - point->x * shot->width) + sqr_value(org->pt.y - point->y * shot->height))) {
                                            org_found = true;
                                            org_id = j;
                                        }

                                        if (nearly_zero(
                                                sqr_value(dst->pt.x - point->x * shot->width) + sqr_value(dst->pt.y - point->y * shot->height))) {
                                            dst_found = true;
                                            dst_id = j;
                                        }
                                    }
            ...
            for (std::map<int, unsigned int>::iterator edge_i2 = edge_i1->second.begin(); edge_i2 != edge_i1->second.end(); ++edge_i2) {
                ...
                if (edge_i2->second >= 1) {
                    ...
                    std::map<int, std::map<int, unsigned int> >::iterator
                        edge_i2_neighbours
                        = picture_edges.find(edge_i2->first);

                    std::map<int, unsigned int>::iterator
                        edge_i1i
                        = edge_i1->second.begin(),
