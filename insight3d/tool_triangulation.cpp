...
void tool_triangulate_surface_reconstruction() {
	...
	CvSubdiv2D* subdivision = cvCreateSubdiv2D(
		CV_SEQ_KIND_SUBDIV2D, sizeof(*subdivision), sizeof(CvSubdiv2DPoint), sizeof(CvQuadEdge2D), mem_storage);
	cvInitSubdivDelaunay2D(subdivision, cvRect(0, 0, shot->width, shot->height));
	...
	for
		ALL(shot->points, i) {
			...
			cvSubdivDelaunay2DInsert(subdivision, cvPoint2D32f(x, y));
		}
		...
		if (CV_IS_SET_ELEM(edge)) {
			...
			for
				ALL(shot->points, j) {
					...
					if (nearly_zero(sqr_value(org->pt.x - point->x * shot->width) + sqr_value(org->pt.y - point->y * shot->height))) {
						org_found = true;
						org_id = j;
					}

					if (nearly_zero(sqr_value(dst->pt.x - point->x * shot->width) + sqr_value(dst->pt.y - point->y * shot->height))) {
						dst_found = true;
						dst_id = j;
					}
				}
			...
			if (org_found && dst_found) {
				...
				// if
				// (detected_edges[shot->points.data[org_id].vertex][shot->points.data[dst_id].vertex]
				// > 0)
				{
	...
	for (std::map<int, std::map<int, unsigned int> >::iterator edge_i1 = picture_edges.begin();
		 edge_i1 != picture_edges.end();
		 ++edge_i1) {
		for (std::map<int, unsigned int>::iterator edge_i2 = edge_i1->second.begin(); edge_i2 != edge_i1->second.end();
			 ++edge_i2) {
			if (edge_i2->first <= edge_i1->first)
				continue;

			if (edge_i2->second >= 1) {
				// find common neighbours of these two vertices to form triangles
				std::map<int, std::map<int, unsigned int> >::iterator edge_i2_neighbours = picture_edges.find(edge_i2->first);

				std::map<int, unsigned int>::iterator edge_i1i = edge_i1->second.begin(),
					...
					if (edge_i1i->first == edge_i2i->first) {
						if (edge_i1i->first != edge_i1->first && edge_i1i->first != edge_i2->first) {
							if (edge_i1i->second >= 1 && edge_i2i->second >= 1) {
								if (inserted.find(std::make_pair<int, std::pair<int, int> >(
										edge_i1->first, std::make_pair<int, int>(edge_i2->first, edge_i1i->first)))
									== inserted.end()) {
									inserted.insert(std::make_pair<int, std::pair<int, int> >(
										...
										// length
										const double len_2d_a = sqrt(distance_sq_2(
														 shot->points.data[edge_i1->first].x * shot->width,
														 shot->points.data[edge_i1->first].y * shot->height,
														 shot->points.data[edge_i2->first].x * shot->width,
														 shot->points.data[edge_i2->first].y * shot->height)),
													 len_2d_b = sqrt(distance_sq_2(
														 shot->points.data[edge_i2->first].x * shot->width,
														 shot->points.data[edge_i2->first].y * shot->height,
														 shot->points.data[edge_i1i->first].x * shot->width,
														 shot->points.data[edge_i1i->first].y * shot->height)),
										...
										// length
										const double len_3d_a = sqrt(distance_sq_3(
...
void tool_triangulation_create() {
	tool_create(UI_MODE_UNSPECIFIED,
		"Triangulation of vertices",
		"Allows to triangulate vertices marked on individual shots by points");
	...
	tool_register_menu_function("Main menu|Modelling|Triangulate, only selected shots|",
		tool_triangulate_vertices_using_selected_shots);
...
void tool_triangulate_vertices_user() {
	action_triangulate_vertices(NULL,
		...
		MVG_MIN_INLIERS_TO_TRIANGULATE_WEAKER,
		true,
		50); // note that we force larger measurement threshold, this should be
...
void compute_vertex_normal_from_pointcloud(const size_t vertex_id, ANNkd_tree* ann_kdtree, size_t* vertices_reindex) {
	...
	for
		ALL(incidence->shot_point_ids, i) {
			...
			// decide orientation
			// note that we're relying on that the T is precomputed,
			// which is stupid
