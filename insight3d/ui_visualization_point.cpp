...
void visualization_point(double x, double y, unsigned int style)
{
    if (style & VISUALIZATION_AUTO) {
        ...
        if (style & VISUALIZATION_SELECTED)
            glColor4d(0.8, 0, 0.8, 1);
        else if (style & VISUALIZATION_FOCUSED)
            glColor4d(1, 1, 1, 1);
        else
            glColor4d(1, 1, 1, 0.7);
        ...
        if (style & VISUALIZATION_FOCUSED)
            glColor3d(0, 0, 0);
        else
            glColor4d(0, 0, 0, 0.7);
        ...
        glLineWidth(4);
        if (style & VISUALIZATION_SELECTED) /*glColor3d(226 / 255.0, 122 / 255.0, 1.0);*/
            glColor3d(0.8, 0, 0.8);
        else
            glColor3d(1, 1, 1);
        glBegin(GL_LINES);
...
{
    if (!outlier)
        glColor4d(0, 0.8, 0, 0.6);
    else
        glColor4d(0.7, 0, 0, 0.6);
    glLineWidth(1.0);
    glBegin(GL_LINES);
    glVertex3d(x, y, -1);
    glVertex3d(rx, ry, -1);
    ...
    // go through all unselected and unfocused 2d points on this shot
        for
            ALL(shot->points, i)
            {
                ...
                if (option_hide_automatic && vertices.data[shot->points.data[i].vertex].vertex_type == GEOMETRY_VERTEX_AUTO)
                    continue;
