...
int geometry_sort_double_indices_secondary_comparator(const void* pi, const void* pj)
{
    ...
    if (i->secondary < j->secondary)
        return -1;
    else if (i->secondary > j->secondary)
        return 1;
    else
        return 0;
}
        ...
        for
            ALL(polygons, polygon_id)
            {
                ...
                if (found) {
                    ...
                    size_t count = 0;
                        for
                            ALL(polygons.data[polygon_id].vertices, i)
                            {
                                count++;
                                if (count >= 2)
                                    break;
    ...
    // on large sparsely connected scenes
        ...
        for
            ALL(vertices_incidence, i)
            {
                ...
                for
                    ALL(vertex_incidence->shot_point_ids, j)
                    {
                        const Double_Index* const index1 = vertex_incidence->shot_point_ids.data + j;

                        for
                            ALL(vertex_incidence->shot_point_ids, k)
                            {
                                if (j == k)
                                    continue;
