                    ...
                    ALL(shots, j)
                    {
                        ...
                        // for (size_t tries = 0; tries < 2000; tries++) // todo this causes overflow because the number of matches can now be greater that the number of features
                        {
                            ...
                            if (found == 2) {
                                const double
                                    d1
                                    = descr_dist_sq(first_feature, neighbours[0]),
                        ...
                        if (use_ransac && correspondences >= 18) {
                            ...
                            for (size_t k = 0; k < correspondences; k++) {
                                feature
                                    *const first_feature
                                    = first_shot->keypoints + matches[2 * k + 0],
                            ...
                            feature
                                *features1_copy
                                = ALLOC(feature, first_shot->keypoints_count),
    ...
    // go through all vertices
        for
            ALL(vertices_incidence, i)
            {
                ...
                bool valid = true;
                for
                    ALL(incidence->shot_point_ids, j)
                    {
...
void matching_features_union(Matching_UF_Nodes& nodes, feature* first, feature* second)
{
    ...
    if (first_root_found && second_root_found && first_root_id == second_root_id)
        return;
