...
#define GUI_FIRE_EVENT(panel, event) \
    if ((panel)->on_##event) {       \
        (panel)->on_##event(panel);  \
    }
...
bool gui_resolve_mousemotion(SDL_Event* event)
{
    ...
    for (size_t i = 0; i < gui_context.panels_count; i++) {
        ...
        if (gui_context.panels[i]->effectively_hidden)
            continue;
        GUI_Panel* const panel = gui_context.panels[i];
...
bool gui_resolve_event(SDL_Event* event)
{
    ...
    case SDL_MOUSEMOTION:
        return gui_resolve_mousemotion(event);
        break;
    case SDL_MOUSEBUTTONDOWN:
        return gui_resolve_mousebuttondown(event);
        break;
    case SDL_MOUSEBUTTONUP:
        return gui_resolve_mousebuttonup(event);
        break;
    }
...
GUI_Panel* gui_new_tab(GUI_Panel* tabs_container, const char* title)
{
    ...
    // find the last tab
    while (last_button->tab_sibling)
        last_button = last_button->tab_sibling;
