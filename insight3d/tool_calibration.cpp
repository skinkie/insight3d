...
// parameter ids
const size_t
    CALIBRATION_IMAGE_MEASUREMENT_THRESHOLD
    = 0,
    ...
    size_t* const calibration_id)
{
    ...
    if (normalize_data) {
        ...
        if (!H_normalization1 || !H_normalization2 || !H_normalization1_inv || !H_normalization2_inv) {
            ...
            if (H_normalization1_inv)
                cvReleaseMat(&H_normalization1_inv);
            if (H_normalization2_inv)
    ...
    *calibration_id = LAST_INDEX(calibrations);
    ...
    for (size_t i = 0; i < points1->cols; i++) {
        ...
        if (CV_MAT_ELEM(*status, signed char, 0, i) == 0)
            continue;
...
void tool_calibration_pair()
{
    ...
    const bool
        normalize_data
        = tool_get_bool(tool_calibration_id, CALIBRATION_NORMALIZE_DATA),
...
bool calibration_add_view(const size_t calibration_id, const size_t shot_id, const double threshold, const bool normalize_data, const bool normalize_A)
{
    ...
    if (ok) {
        ...
        // denormalize P
        if (normalize_data)
            cvMatMul(H_normalization_inv, P, P);
    ...
    // release resources
    if (normalize_data)
        cvReleaseMat(&H_normalization_inv);
    FREE(inliers);
    FREE(points_indices);
    cvReleaseMat(&points);
    cvReleaseMat(&vertices);
    ...
    const double distance_threshold)
{
    ...
    // go through all pairs
        ...
        for (sufficient_count = 0; sufficient_count < best_count; sufficient_count++) {
            if (best_corr[sufficient_count] < 8)
                break;
        }
        ...
        memset(vertex_count, 0, sizeof(size_t) * shots.count);
                for
                    ALL(calibration->Xs, i)
                    {
                        ...
                        for
                            ALL(vertices_incidence.data[vertex_id].shot_point_ids, j)
                            {
                                ...
                                if (shots.data[index->primary].partial_calibration)
                                    continue;
                ...
                size_t sufficient_count; // how many have at least 8 correspondences between them
                for (sufficient_count = 0; sufficient_count < best_count; sufficient_count++) {
                    if (best_corr[sufficient_count] < 6)
                        break;
                }
    ...
    const double distance_threshold)
{
    ...
    // copy all calibrated projection matrices into workspace
        for
            ALL(calibrations.data[calibration_id].Ps, i)
            {
    ...
    const bool normalize_data, const bool normalize_A)
{
    ...
    if (publish_triangulation_data_from_calibration(calibration_id, vertices_incidence.data[vertex_id], vertex_id, projection_matrices_original, points, indices)) {
        ...
        if (
            points->cols >= 2 && (X = mvg_triangulation_RANSAC(normalize_data ? (const CvMat**)projection_matrices_normalized : projection_matrices_original, points, false, normalize_A, min_inliers, min_inliers, MVG_RANSAC_TRIANGULATION_TRIALS, measurement_threshold * scale, inliers)
            ...
            ALL(calibration->Xs, i)
            {
                ...
                for
                    ALL(vertices_incidence.data[vertex_id].shot_point_ids, j)
                    {
                        ...
                        {
                            if (!(IS_SET(calibration->Ps.data[shots_reindex[index->primary]].points_meta, index->secondary)))
                                continue;
