    ...
    memset(vertices_reindex, 0, sizeof(size_t) * vertices.count);
        ...
        for
            ALL(polygons, i)
            {
                ...
                for
                    ALL(polygons.data[i].vertices, j)
                    {
                ...
                for
                    ALL(vertices, i)
                    {
                        ...
                        if (!vertex->reconstructed)
                            continue;
                        if (restrict_vertices_by_group && vertex->group != restrict_vertices_by_group)
                            continue;
                ...
                for
                    ALL(vertices, i)
                    {
                        ...
                        if (restrict_vertices_by_group && vertex->group != restrict_vertices_by_group)
                            continue;
                        if (
                            inside_interval(vertex->color[0], 0, 1) && inside_interval(vertex->color[1], 0, 1) && inside_interval(vertex->color[2], 0, 1)) {
                            vrml_output
                                << vertex->color[0] << " "
                                << vertex->color[1] << " "
                ...
                for
                    ALL(polygons, i)
                    {
                        for
                            ALL(polygons.data[i].vertices, j)
                            {
                                if (!vertices.data[polygons.data[i].vertices.data[j].value].reconstructed)
                                    continue;
...
bool geometry_export_sandy3d(const char* filename, Vertices& vertices, Polygons_3d& polygons, bool export_vertices)
{
    ...
    char
        *class_name
        = interface_filesystem_extract_filename(filename),
    ...
    as_output << "{" << std::endl;
    as_output << "import sandy.primitive.Primitive3D;" << std::endl
              << std::endl;
    ...
    as_output << "{" << std::endl;
    as_output << "super( p_Name ) ;" << std::endl
              << std::endl;
