    ...
    for (int tries = 0; tries <= total; tries++) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = i; j < 4; j++) {
                ...
                if (i != j)
                    OPENCV_ELEM(Q_temp, j, i) = OPENCV_ELEM(solution, si, 0);
                si++;
            }
        }
        ...
        if (!affine) {
            ...
            // affine
            for (int i = 0; i < 4; i++) {
                OPENCV_ELEM(H_rectify, 3, i) = OPENCV_ELEM(pi_inf, i, 0);
            }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (decomposed) {
                const double PP_distance = sqrt(
                    sq(OPENCV_ELEM(rectified_K, 0, 2) - principal_points[2 * i + 0]) + sq(OPENCV_ELEM(rectified_K, 1, 2) - principal_points[2 * i + 1]));
