...
void opencv_vertex_projection_visualization(const CvMat* const P, const double x, const double y, const double z, double reprojection[])
{
    ...
    if (w == 0)
        w = 0.00001;
    reprojection[0] = (OPENCV_ELEM(P, 0, 0) * x + OPENCV_ELEM(P, 0, 1) * y + OPENCV_ELEM(P, 0, 2) * z + OPENCV_ELEM(P, 0, 3)) / w;
    reprojection[1] = (OPENCV_ELEM(P, 1, 0) * x + OPENCV_ELEM(P, 1, 1) * y + OPENCV_ELEM(P, 1, 2) * z + OPENCV_ELEM(P, 1, 3)) / w;
}
...
void opencv_vertex_projection_visualization(const CvMat* const P, const double x, const double y, const double z, const double w, double reprojection[])
{
    ...
    if (v == 0)
        v = 0.00001;
    reprojection[0] = (OPENCV_ELEM(P, 0, 0) * x + OPENCV_ELEM(P, 0, 1) * y + OPENCV_ELEM(P, 0, 2) * z + OPENCV_ELEM(P, 0, 3) * w) / v;
    reprojection[1] = (OPENCV_ELEM(P, 1, 0) * x + OPENCV_ELEM(P, 1, 1) * y + OPENCV_ELEM(P, 1, 2) * z + OPENCV_ELEM(P, 1, 3) * w) / v;
}
...
{
    double w = OPENCV_ELEM(P, 2, 0) * OPENCV_ELEM(X, 0, 0) + OPENCV_ELEM(P, 2, 1) * OPENCV_ELEM(X, 1, 0) + OPENCV_ELEM(P, 2, 2) * OPENCV_ELEM(X, 2, 0) + OPENCV_ELEM(P, 2, 3) * OPENCV_ELEM(X, 3, 0);

    if (w == 0)
        w = 0.00001;

    reprojection[0] = (OPENCV_ELEM(P, 0, 0) * OPENCV_ELEM(X, 0, 0) + OPENCV_ELEM(P, 0, 1) * OPENCV_ELEM(X, 1, 0) + OPENCV_ELEM(P, 0, 2) * OPENCV_ELEM(X, 2, 0) + OPENCV_ELEM(P, 0, 3) * OPENCV_ELEM(X, 3, 0)) / w;

    reprojection[1] = (OPENCV_ELEM(P, 1, 0) * OPENCV_ELEM(X, 0, 0) + OPENCV_ELEM(P, 1, 1) * OPENCV_ELEM(X, 1, 0) + OPENCV_ELEM(P, 1, 2) * OPENCV_ELEM(X, 2, 0) + OPENCV_ELEM(P, 1, 3) * OPENCV_ELEM(X, 3, 0)) / w;
}
