...
bool ui_inspection_ground_initialization()
{
    ...
    if (visualization_state.ground == NULL)
        return false;
...
void ui_update_inspection(const Uint32 delta_time)
{
    ...
    const double
        camera_step
        = ui_state.inspection_camera_movement_speed * (delta_time / 1000.0),
    ...
    if (no_revert) {
        if (ui_state.keys[SDL_SCANCODE_LEFT])
            visualization_state.R[Y] += camera_rot;
        if (ui_state.keys[SDL_SCANCODE_RIGHT])
            visualization_state.R[Y] -= camera_rot;
    } else {
        if (ui_state.keys[SDL_SCANCODE_LEFT])
            visualization_state.R[Y] -= camera_rot;
    ...
    if (!sdl_shift_pressed()) {
        ...
        if (ui_state.keys[SDLK_UP])
            visualization_state.R[X] -= camera_rot;
        if (ui_state.keys[SDLK_DOWN])
            visualization_state.R[X] += camera_rot;
    }
