...
bool geometry_load_project(const char* filename)
{
    ...
    for (size_t i = 0; i < vertices_count; i++) {
        ...
        switch (vertex_type) {
        case GEOMETRY_VERTEX_AUTO:
            vertices.data[vertex_id].vertex_type = GEOMETRY_VERTEX_AUTO;
            break;
        case GEOMETRY_VERTEX_EQUIVALENCE:
            vertices.data[vertex_id].vertex_type = GEOMETRY_VERTEX_EQUIVALENCE;
            break;
        case GEOMETRY_VERTEX_USER:
            vertices.data[vertex_id].vertex_type = GEOMETRY_VERTEX_USER;
            break;
        }
    }
    ...
    for (size_t i = 0; i < shots_count; i++) {
        ...
        switch (status) {
        case GEOMETRY_INFO_DEDUCED:
            shot->info_status = GEOMETRY_INFO_DEDUCED;
            break;
        case GEOMETRY_INFO_LOADED:
            shot->info_status = GEOMETRY_INFO_LOADED;
            break;
        case GEOMETRY_INFO_NOT_LOADED:
            shot->info_status = GEOMETRY_INFO_NOT_LOADED;
            break;
        }
...
void geometry_loader_SAX_start_element(geometry_loader_SAX_state* state, const xmlChar* name, const xmlChar** attrs)
{
    if (xmlStrcmp(name, XML_ELEM_SHOT) == 0) {
        ...
        // mark as calibrated
        if (attr)
            state->current_shot->calibrated = true;
        ...
        while (*attr != NULL) {
            ...
            // x coordinate
            if (xmlStrcmp(*attr, XML_ATTR_X) == 0)
                state->current_shot->T[0] = atof((char*)value); // note conversion
            // y coordinate
            else if (xmlStrcmp(*attr, XML_ATTR_Y) == 0)
                state->current_shot->T[1] = atof((char*)value); // note conversion
            // z coordinate
            else if (xmlStrcmp(*attr, XML_ATTR_Z) == 0)
                state->current_shot->T[2] = atof((char*)value); // note conversion
        ...
        while (*attr != NULL) {
            ...
            // x angle
            if (xmlStrcmp(*attr, XML_ATTR_X) == 0)
                state->current_shot->R_euler[0] = deg2rad(atof((char*)value)); // note conversion
            // y angle
            else if (xmlStrcmp(*attr, XML_ATTR_Y) == 0)
                state->current_shot->R_euler[1] = deg2rad(atof((char*)value)); // note conversion
            // z angle
            else if (xmlStrcmp(*attr, XML_ATTR_Z) == 0)
                state->current_shot->R_euler[2] = deg2rad(atof((char*)value)); // note conversion
        ...
        while (*attr != NULL) {
            ...
            // fovx angle
            if (xmlStrcmp(*attr, XML_ATTR_FOVX) == 0)
                state->current_shot->fovx = atof((char*)value); // note conversion
        ...
        while (*attr != NULL) {
            ...
            // film back size
            if (xmlStrcmp(*attr, XML_ATTR_FILM_BACK) == 0)
                state->first_film_back = atof((char*)value); // note conversion
        ...
        while (*attr != NULL) {
            ...
            // x coordinate
            if (xmlStrcmp(*attr, XML_ATTR_X) == 0)
                x = atof((char*)value); // note conversion
            // y coordinate
            else if (xmlStrcmp(*attr, XML_ATTR_Y) == 0)
                y = atof((char*)value); // note conversion
            // z coordinate
            else if (xmlStrcmp(*attr, XML_ATTR_Z) == 0)
                z = atof((char*)value); // note conversion
...
bool geometry_loader_vertices(const char* txt_filename, Vertices& vertices, size_t group /*= 0*/)
{
    ...
    if (input_vertices) {
        // read all it's lines
        size_t vertex_input_id = 0;
        double x, y, z;
        while (input_vertices >> x >> y >> z) {
...
bool geometry_loader_points(const char* pictures_filename, const char* tracks_filename, Shots& shots, Vertices& vertices, size_t group /*= 0*/)
{
    ...
    if (input_tracks) {
        ...
        while (input_tracks >> picture_id >> track_id >> x >> y) {
            ...
            if (!IS_SET(index_in_shots, picture_id))
                continue;
