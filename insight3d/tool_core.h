...
enum Tool_Parameter_Type { TOOL_PARAMETER_REAL,
    TOOL_PARAMETER_INT,
    TOOL_PARAMETER_ENUM,
    TOOL_PARAMETER_BOOL };
...
// button type
enum Tool_Action_Type { TOOL_ACTION_NONE,
    TOOL_ACTION_SHOW,
    TOOL_ACTION_FILE_DIALOG,
    TOOL_ACTION_FUNCTION_CALL };
...
#define TOOL_PASS_CLICK(tool_name)          \
    tool_##tool_name##_click(x, y, button); \
    return // note it's not possible to use this macro without enclosing it into curly braces
#define TOOL_PASS_MOUSE_DOWN(tool_name) return tool_##tool_name##_mouse_down(x, y, button)
#define TOOL_PASS_KEY(tool_name) \
    tool_##tool_name##_key();    \
    return
#define TOOL_PASS_MOVE(tool_name)  \
    tool_##tool_name##_move(x, y); \
    return
#define TOOL_PASS_DRAGGING(tool_name)          \
    tool_##tool_name##_dragging(x, y, button); \
    return
#define TOOL_PASS_DRAGGING_DONE(tool_name)                    \
    tool_##tool_name##_dragging_done(x1, y1, x2, y2, button); \
    return
