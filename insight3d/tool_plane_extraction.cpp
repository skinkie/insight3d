...
double* tool_plane_extraction(Vertices& vertices, double threshold /*= 0.5*/, bool flatten_inliers /*= false*/, size_t group /*= 0*/)
{
    ...
    size_t reconstructed_vertices_count = 0;
        ...
        for
            ALL(vertices, i)
            {
                if (vertices.data[i].reconstructed && ++reconstructed_vertices_count == 3)
                    break;
            }

        if (reconstructed_vertices_count < 3)
            return NULL;
        ...
        while (tries++ < 1000) {
            ...
            if (
                nearly_zero(distance_sq_3(picked[0], picked[1])) || nearly_zero(distance_sq_3(picked[0], picked[2])) || nearly_zero(distance_sq_3(picked[1], picked[2]))) {
                continue;
            }
...
double* tool_plane_extraction_subset(Vertices& vertices, size_t* ids, size_t count)
{
    ...
    for (size_t i = 0; i < count; i++) {
        if (vertices.data[ids[i]].reconstructed && ++reconstructed_vertices_count == 3)
            break;
    }
    ...
    while (tries++ < 20) {
        ...
        // put a plane through these points
        double plane[4];
        if (!plane_from_three_points(picked[0], picked[1], picked[2], plane))
            continue;
