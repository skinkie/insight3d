    ...
    for (size_t i = 0; i < shots.count; i++) {
        if (!nearly_zero(shots.data[i].T[W])) {
			...
			*/
            ...
            const double
                normalize_10
                = OPENCV_ELEM(P_pseudoinverse, 3, 2) + OPENCV_ELEM(P_pseudoinverse, 3, 0) * shots.data[i].width,
                normalize_01 = OPENCV_ELEM(P_pseudoinverse, 3, 2) + OPENCV_ELEM(P_pseudoinverse, 3, 1) * shots.data[i].height,
                normalize_11 = OPENCV_ELEM(P_pseudoinverse, 3, 2) + OPENCV_ELEM(P_pseudoinverse, 3, 0) * shots.data[i].width + OPENCV_ELEM(P_pseudoinverse, 3, 1) * shots.data[i].height;

            for (size_t k = 0; k < 3; k++) {
                shots.data[i].visualization_pyr_00[k] = OPENCV_ELEM(P_pseudoinverse, k, 2) / OPENCV_ELEM(P_pseudoinverse, 3, 2);

                shots.data[i].visualization_pyr_10[k] = (OPENCV_ELEM(P_pseudoinverse, k, 2) + shots.data[i].width * OPENCV_ELEM(P_pseudoinverse, k, 0))
                    / normalize_10;

                shots.data[i].visualization_pyr_01[k] = (OPENCV_ELEM(P_pseudoinverse, k, 2) + shots.data[i].height * OPENCV_ELEM(P_pseudoinverse, k, 1))
                    / normalize_01;

                shots.data[i].visualization_pyr_11[k] = (OPENCV_ELEM(P_pseudoinverse, k, 2) + shots.data[i].width * OPENCV_ELEM(P_pseudoinverse, k, 0) + shots.data[i].height * OPENCV_ELEM(P_pseudoinverse, k, 1))
                    / normalize_11;
            }
    ...
    // first check if we have enough points
        ...
        for
            ALL(vertices, i)
            {
                ...
                if (vertex->reconstructed) {
                    if (use_man && vertex->vertex_type != GEOMETRY_VERTEX_USER)
                        continue;
...
{
    if (visualization_state.max_dev == 0)
        return x;
        ...
        for
            ALL(vertices, i)
            {
                ...
                if (vertices.data[i].group) {
                    ...
                    if (ui_state.groups.data[vertices.data[i].group].hidden)
                        continue;
                }

                // optionally skip generated vertices
                if (option_hide_automatic && vertices.data[i].vertex_type == GEOMETRY_VERTEX_AUTO)
                    continue;
    ...
    // go through all polygons
        for
            ALL(polygons, i)
            {
                ...
                if (!query_is_polygon_reconstructed(*polygon, vertices))
                    continue;
            ...
            ALL(shots, i)
            {
                ...
                for
                    ALL(shot->contours, j)
                    {
                        ...
                        if (!query_is_contour_reconstructed(*contour, shot->points, vertices))
                            continue;

                        // go through all 2d vertices of this polygon
                        for
                            ALL(contour->vertices, k)
                            {
...
void visualization_shot_image(Shot& shot)
{
    ...
    if (
        image_loader_nonempty_handle(shot.image_loader_request) && image_loader_request_ready(shot.image_loader_request)) {
        ...
        if (image_loader_opengl_upload_ready_dual(shot.image_loader_request, &full_texture, &low_texture)) {
            if (!full_texture) {
                ...
                if (visualization_state.continuous_loading_alpha < 0)
                    visualization_state.continuous_loading_alpha = 0;
        ...
        for
            ALL(current_shot->points, i)
            {
                ...
                if (query_find_point_on_shot_by_vertex_id(dual_shot_id, vertex_id, corresponding_point_id)) {
                    ...
                    if (!highlighted) {
                        if (weight > 0)
                            glColor4d(r, g, b, 0.2 + 0.8 * weight);
                        else
                            glColor4d(1, 1, 1, 0.3);
                    } else {
                        glColor4d(1, 0, 1, 1);
                    ...
                    if (!highlighted) {
                        if (weight > 0)
                            glColor4d(r, g, b, 0.4 + 0.6 * weight);
                        else
                            glColor4d(1, 1, 1, 0.4);
                    } else {
                        glColor4d(1, 0, 1, 1);
...
void visualization_rect_with_shadow(double left, double top, double right, double bottom, double texture_left, double texture_top, double texture_right, double texture_bottom, double alpha)
{
    ...
    glBegin(GL_POLYGON);
    glTexCoord2d(texture_left, texture_top);
    glVertex3d(left, top, -1);
    glTexCoord2d(texture_right, texture_top);
    glVertex3d(right, top, -1);
    glTexCoord2d(texture_right, texture_bottom);
    glVertex3d(right, bottom, -1);
    glTexCoord2d(texture_left, texture_bottom);
...
void visualization_shot_contours(const size_t shot_id)
{
    ...
    // go through all 2d polygons on this shot
        for
            ALL(shots.data[shot_id].contours, i)
            {
                // setup polygon
                glBegin(GL_LINE_LOOP);
                ...
                // go through all vertices of this polygon
                for
                    ALL(shots.data[shot_id].contours.data[i].vertices, j)
                    {
