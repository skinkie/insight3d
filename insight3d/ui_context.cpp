    ...
    double max_width = 0;
        ...
        for
            ALL(context_state.items, i)
            {
                ...
                const double
                    item_width
                    = visualization_calc_dx(UI_CONTEXT_SCALE * item->width),
                ...
                if (item->content == UI_CONTEXT_THUMBNAIL) {
                    ...
                    if (image_loader_opengl_upload_ready(item->request, &texture, &tx, &ty, &sx, &sy)) {
                        ...
                        glBegin(GL_POLYGON);
                        glTexCoord2d(tx, sy);
                        glVertex3d(x + distance_to_cursor + border_x, y - symbol_height / 2 + y_level, -1);
                        glTexCoord2d(sx, sy);
                        glVertex3d(x + distance_to_cursor + border_x + item_width, y - symbol_height / 2 + y_level, -1);
                        glTexCoord2d(sx, ty);
                        glVertex3d(x + distance_to_cursor + border_x + item_width, y - symbol_height / 2 + y_level + item_height, -1);
                        glTexCoord2d(tx, ty);
                        glVertex3d(x + distance_to_cursor + border_x, y - symbol_height / 2 + y_level + item_height, -1);
                        glEnd();
                    ...
                    GLuint texture;
                    if (
                        image_loader_nonempty_handle(shots.data[ui_state.current_shot].image_loader_request) && image_loader_opengl_upload_ready(shots.data[ui_state.current_shot].image_loader_request, &texture)) {
                        double tx, ty, sx, sy;
                ...
                if (drawn) {
                    if (item->decoration == UI_CONTEXT_CROSSHAIR) {
                        ...
                        const double
                            cx
                            = x + distance_to_cursor + border_x + item_width / 2,
...
{
    if (context_state.count >= UI_CONTEXT_MAX_SHOWN_ITEMS)
        return;
